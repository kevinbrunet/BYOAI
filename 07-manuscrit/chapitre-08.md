...
Un exemple daté suffit à montrer la vitesse du mouvement. Le Model Context Protocol — le mécanisme par lequel un nombre croissant d'agents d'entreprise appellent leurs outils, le châssis évoqué au chapitre 6 — a publié fin juillet 2026 la plus grosse révision de sa spécification depuis son lancement : les serveurs MCP y sont désormais traités comme des serveurs de ressources OAuth 2.0, avec jetons liés à une audience précise, PKCE, et une extension d'autorisation gérée par l'entreprise qui centralise le contrôle via le fournisseur d'identité plutôt que serveur par serveur[^8-6]. Ce n'est pas l'architecture de ce chapitre qui rattrape un standard existant : c'est un standard qui converge, sous la pression de vraies attaques en production, vers ce qu'il décrit — identité vérifiée indépendamment du mandat, jetons portant leur audience, autorisation calculée avant que la requête n'atteigne le service. Dans dix-huit mois, des noms auront changé. Voici ce qui n'aura pas changé, et qui constitue la commande à passer à vos équipes dès maintenant, quelle que soit la brique retenue :
...
[^8-1]: ✓ Identité de *workload* : mTLS avec identités SPIFFE, délivrées et renouvelées automatiquement par SPIRE. SPIFFE/SPIRE est un projet *graduated* de la CNCF (maturité confirmée).

[^8-2]: ✓ *OAuth 2.0 Token Exchange*, RFC 8693 (IETF, finalisée) : échange d'un jeton contre un jeton dérivé portant une claim `act` (« agit pour ») empilable, préservant la chaîne de délégation. ⚠ Le support natif du claim `act` dans Keycloak est en discussion ouverte — l'assemblage bout-en-bout reste jeune.

[^8-3]: ⚠ Kagenti et AuthBridge (Red Hat) : architecture de référence et démonstration, **pas** un produit supporté sur étagère. Séquence et détails à revérifier impérativement sur les publications Red Hat avant publication — la fiche source signale une hallucination partielle corrigée dans la conversation d'origine.

[^8-4]: ✓ Biscuit (fondation Eclipse) : jeton porteur de ses droits (faits et règles logiques), atténuable hors ligne par ajout de *caveats* scellés cryptographiquement (on ne peut que restreindre). En production chez Clever Cloud (hébergeur européen) depuis 2021.

[^8-5]: ✓ Le groupe WIMSE (IETF) travaille l'identité de workload mais ne définit pas l'autorité de délégation — le « trou » que pointe ce chapitre. ~ Chiffres d'adoption OAuth/MCP écartés (non confirmés). RFC 8705 (liaison cryptographique des deux couches) volontairement omise du corps (marqueur trop fragile) — à réintroduire en Annexe A si revérifiée.

[^8-6]: ✓ Spécification MCP, candidate de sortie du 28/07/2026 (plus grosse révision depuis le lancement du protocole) : serveurs traités comme des serveurs de ressources OAuth 2.0, alignement OAuth 2.1/PKCE, jetons liés à une audience. ✓ Extension "Enterprise-Managed Authorization" promue au statut stable : contrôle d'accès centralisé via le fournisseur d'identité de l'entreprise, flux "zéro contact" remplaçant les popups de consentement serveur par serveur. ✓ MCP donné par Anthropic à l'Agentic AI Foundation (Linux Foundation) en décembre 2025. Vérifié le 17/07/2026 sur blog.modelcontextprotocol.io et techcommunity.microsoft.com.
...
- **Ajout 2026-07-17 (décision Kevin, propagation depuis ch. 6)** : paragraphe MCP inséré dans "Ce qu'un DSI doit décider maintenant", entre le paragraphe sur le trou WIMSE et "Dans dix-huit mois, des noms auront changé" (repris comme phrase de clôture du nouveau paragraphe). Corrige au passage le renoncement noté en [^8-5] ("chiffres d'adoption OAuth/MCP écartés, non confirmés") : la recherche du 17/07 fournit cette fois une source vérifiable et datée (spec candidate MCP du 28/07/2026, gouvernance Linux Foundation), remplaçant l'ancien chiffre d'adoption non confirmé par un fait de spécification confirmé. Note [^8-6] ajoutée. Voir [posture-ouverture-second-axe](../01-concepts/posture-ouverture-second-axe.md) pour le détail complet de la recherche MCP.
